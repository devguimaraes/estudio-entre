...
interface SeboBookCardProps {
  livro: LivroSebo;
}

export default function SeboBookCard({ livro }: SeboBookCardProps) {
  return (
    <article className="sebo-card group rounded-[2rem] bg-white p-8 shadow-lg shadow-bordo/[0.03] transition-all duration-500 hover:-translate-y-1 hover:shadow-xl hover:shadow-bordo/[0.06]">
      {/* Genre dot + label */}
      ...
      <div className="mt-7 flex items-center justify-between border-t border-bordo/[0.04] pt-6">
        <span className="font-display text-2xl font-black text-orange">{livro.valor}</span>
        <a
